            ...
            <div class="col-xl-12 col-lg-12 col-md-12">
                <div class="card">
                    <div class="card-header py-3 bg-transparent border-bottom-0">
                        <h5 class="m-0 text-primary">Update Material Collection Facility</h5>
                    </div>
                    <form method="post" action="{{ route('mcf.update', $data->id) }}">
                        @csrf
                        @method("PUT")
                        <div class="card-body">
                            <div class="row mb-3">
                                                ...
                                                @if($corp->lsg_type == 'CO')
                                                    @php $c = 1 @endphp
                                                        <tr>
                                                            ...
                                                            @foreach($questions as $key => $question)
                                                            <td>
                                                                <input type="{{ $question->type }}" name="q{{$c++}}[]" min="0" class="form-control form-control-sm rounded-0 {{ $question->position }}" value="{{ DB::table('mcf_data')->where('id', $corp->id)->value('q'.$key+1) }}" placeholder="{{ $question->placeholder }}" />
                                                            </td>
                                                            @endforeach
                                                        </tr>
                                                    @php $k++; @endphp
                                            ...
                                            @foreach($records as $key => $mun)
                                                @if($mun->lsg_type == 'MP')
                                                    ...
                                                    <tr>
                                                        ...
                                                        @foreach($questions as $key => $question)
                                                        <td>
                                                            <input type="{{ $question->type }}" name="q{{$c++}}[]" min="0" class="form-control form-control-sm rounded-0 {{ $question->position }}" value="{{ DB::table('mcf_data')->where('id', $mun->id)->value('q'.$key+1) }}" placeholder="{{ $question->placeholder }}" />
                                                        </td>
                                                        @endforeach
                                                    </tr>
                                                    @php $k++; @endphp
                                            ...
                                            @foreach($records as $key => $gp)
                                                @if($gp->lsg_type == 'GP')
                                                    ...
                                                    <tr>
                                                        ...
                                                        @foreach($questions as $key => $question)
                                                        <td>
                                                            <input type="{{ $question->type }}" name="q{{$c++}}[]" min="0" class="form-control form-control-sm rounded-0 {{ $question->position }}" value="{{ DB::table('mcf_data')->where('id', $gp->id)->value('q'.$key+1) }}" placeholder="{{ $question->placeholder }}" />                                                        
                                                        </td>
                                                        @endforeach
                                                    </tr>
                                                    @php $k++; @endphp
