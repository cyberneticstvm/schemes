                        ...
                        <div class="row mb-3">
                            <div class="col-md-12 table-responsive">
                                <table class="table table-bordered table-sm table-striped table-hover">
                                    ...
                                    <tbody>
                                        @php $c = 1; $a= 0; $b = 0; $c = 0; $d = 0; $e = 0; $e1 = 0; $f = 0; $g = 0; $h = 0; $i = 0; $i1 = 0; $j = 0; $k = 0; $l = 0; $m = 0; $n = 0; $o = 0; $p = 0; $p1 = 0; $q = 0; $r = 0; @endphp
                                        @forelse($data as $key => $row)
                                            @php
                                                $rrf_co = DB::table('rrf_data as md')->leftJoin('rrf_masters as m', 'm.id', 'md.rrf_id')->where('md.lsg_type', 'CO')->where('m.district', $row->id)->sum('md.q1');
                                                $rrf_mp = DB::table('rrf_data as md')->leftJoin('rrf_masters as m', 'm.id', 'md.rrf_id')->where('md.lsg_type', 'MP')->where('m.district', $row->id)->sum('md.q1');
                                                $rrf_mp_reqd = DB::table('municipalities as m')->leftJoin('districts as d', 'm.district', 'd.id')->where('m.district', $row->id)->sum('m.rrf_reqd');
                                                $rrf_gp = DB::table('rrf_data as md')->leftJoin('rrf_masters as m', 'm.id', 'md.rrf_id')->where('md.lsg_type', 'GP')->where('m.district', $row->id)->sum('md.q1');
                                                $rrf_gp_reqd = DB::table('gramapanchayats as g')->leftJoin('districts as d', 'g.district', 'd.id')->where('g.district', $row->id)->sum('g.rrf_reqd');
                                                $rrf_bl = DB::table('rrf_data as md')->leftJoin('rrf_masters as m', 'm.id', 'md.rrf_id')->where('md.lsg_type', 'BL')->where('m.district', $row->id)->sum('md.q1');
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right">{{ $row->rrf_reqd }}</td>
                                                <td class="text-right">{{ $rrf_co }}</td>
                                                <td class="text-right">{{ ($row->rrf_reqd > 0 && $rrf_co > 0) ? number_format((100/$row->rrf_reqd)*$rrf_co, 2) : 0.00 }}%</td>
                                                ...
                                                <td class="text-right">{{ $rrf_mp_reqd }}</td>
                                                <td class="text-right">{{ $rrf_mp }}</td>
                                                <td class="text-right">{{ ($rrf_mp_reqd > 0 && $rrf_mp > 0) ? number_format((100/$rrf_mp_reqd)*$rrf_mp, 2) : 0.00 }}%</td>
                                                ...
                                                <!--<td class="text-right">{{ $rrf_gp_reqd }}</td>-->
                                                <td class="text-right">{{ $rrf_gp }}</td>
                                                <!--<td class="text-right">{{ ($rrf_gp_reqd > 0 && $rrf_gp > 0) ? number_format((100/$rrf_gp_reqd)*$rrf_gp, 2) : 0.00 }}%</td>-->

                                                <td class="text-right">{{ $p1 = DB::table('blocks as b')->leftJoin('districts as d', 'b.district', 'd.id')->where('b.district', $row->id)->count('b.id') }}</td>
                                                <td class="text-right">{{ $rrf_bl_reqd }}</td>
                                                <td class="text-right">{{ $rrf_bl }}</td>
                                                <td class="text-right">{{ ($rrf_bl_reqd > 0 && $rrf_bl > 0) ? number_format((100/$rrf_bl_reqd)*$rrf_bl, 2) : 0.00 }}%</td>
                                            </tr>
                                            @php 
                                                $a += $row->ccount;
                                                $b += $row->rrf_reqd;
                                        ...
                                        @empty
                                        @endforelse
                                        <tr><td class="font-weight-bold text-right">Total</td><td class="font-weight-bold text-right">{{ $a }}</td><td class="font-weight-bold text-right">{{ $b }}</td><td class="font-weight-bold text-right">{{ $c }}</td><td class="font-weight-bold text-right">{{ ($b > 0) ? number_format((100/$b)*$c, 2) : 0 }}%</td><td class="font-weight-bold text-right">{{ $e }}</td><td class="font-weight-bold text-right">{{ $f }}</td><td class="font-weight-bold text-right">{{ $g }}</td><td class="font-weight-bold text-right">{{ ($f > 0) ? number_format((100/$f)*$g, 2) : 0 }}%</td><td class="font-weight-bold text-right">{{ $i }}</td><!--<td class="font-weight-bold text-right">{{ $j }}</td>--><td class="font-weight-bold text-right">{{ $k }}</td><!--<td class="font-weight-bold text-right">{{ ($j > 0) ? number_format((100/$j)*$k, 2) : 0 }}%</td>--><td class="font-weight-bold text-right">{{ $p }}</td><td class="font-weight-bold text-right">{{ $q }}</td><td class="font-weight-bold text-right">{{ $r }}</td><td class="font-weight-bold text-right">{{ ($q > 0) ? number_format((100/$q)*$r, 2) : 0 }}%</td></tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
